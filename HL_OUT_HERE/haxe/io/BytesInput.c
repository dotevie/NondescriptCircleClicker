// Generated by HLC 4.3.1 (HL v5)
#define HLC_BOOT
#include <hlc.h>
#include <haxe/io/BytesInput.h>
#include <haxe/io/Bytes.h>
#include <haxe/io/Error.h>
extern hl_type t$_i32;
extern venum* g$haxe_io_Error_OutsideBounds;
#include <haxe/io/Eof.h>
extern hl_type t$haxe_io_Eof;
void haxe_io_Eof_new(haxe__io__Eof);
#include <hl/natives.h>

void haxe_io_BytesInput_new(haxe__io__BytesInput r0,haxe__io__Bytes r1,vdynamic* r2,vdynamic* r3) {
	venum *r8;
	vdynamic *r5;
	vbyte *r9;
	int r6, r7;
	if( r2 ) goto label$8de92c3_1_4;
	r6 = 0;
	r5 = hl_alloc_dynamic(&t$_i32);
	r5->v.i = r6;
	r2 = r5;
	label$8de92c3_1_4:
	if( r3 ) goto label$8de92c3_1_11;
	if( r1 == NULL ) hl_null_access();
	r6 = r1->length;
	r7 = r2 ? r2->v.i : 0;
	r6 = r6 - r7;
	r5 = hl_alloc_dynamic(&t$_i32);
	r5->v.i = r6;
	r3 = r5;
	label$8de92c3_1_11:
	r6 = r2 ? r2->v.i : 0;
	r7 = 0;
	if( r6 < r7 ) goto label$8de92c3_1_23;
	r6 = r3 ? r3->v.i : 0;
	r7 = 0;
	if( r6 < r7 ) goto label$8de92c3_1_23;
	r6 = r2 ? r2->v.i : 0;
	r7 = r3 ? r3->v.i : 0;
	r6 = r6 + r7;
	if( r1 == NULL ) hl_null_access();
	r7 = r1->length;
	if( r7 >= r6 ) goto label$8de92c3_1_25;
	label$8de92c3_1_23:
	r8 = (venum*)g$haxe_io_Error_OutsideBounds;
	hl_throw((vdynamic*)r8);
	label$8de92c3_1_25:
	r9 = r1->b;
	r0->b = r9;
	r6 = r2 ? r2->v.i : 0;
	r0->pos = r6;
	r6 = r3 ? r3->v.i : 0;
	r0->len = r6;
	r6 = r3 ? r3->v.i : 0;
	r0->totlen = r6;
	return;
}

int haxe_io_BytesInput_readByte(haxe__io__BytesInput r0) {
	haxe__io__Eof r4;
	vbyte *r5;
	int r2, r3;
	r2 = r0->len;
	r3 = 0;
	if( r2 != r3 ) goto label$8de92c3_2_6;
	r4 = (haxe__io__Eof)hl_alloc_obj(&t$haxe_io_Eof);
	haxe_io_Eof_new(r4);
	hl_throw((vdynamic*)r4);
	label$8de92c3_2_6:
	r2 = r0->len;
	--r2;
	r0->len = r2;
	r5 = r0->b;
	r2 = r0->pos;
	r3 = r2;
	++r2;
	r0->pos = r2;
	r3 = *(unsigned char*)(r5 + r3);
	return r3;
}

int haxe_io_BytesInput_readBytes(haxe__io__BytesInput r0,haxe__io__Bytes r1,int r2,int r3) {
	venum *r7;
	haxe__io__Eof r8;
	vbyte *r9, *r10;
	int r5, r6;
	r6 = 0;
	if( r2 < r6 ) goto label$8de92c3_3_8;
	r6 = 0;
	if( r3 < r6 ) goto label$8de92c3_3_8;
	r5 = r2 + r3;
	if( r1 == NULL ) hl_null_access();
	r6 = r1->length;
	if( r6 >= r5 ) goto label$8de92c3_3_10;
	label$8de92c3_3_8:
	r7 = (venum*)g$haxe_io_Error_OutsideBounds;
	hl_throw((vdynamic*)r7);
	label$8de92c3_3_10:
	r5 = r0->len;
	r6 = 0;
	if( r5 != r6 ) goto label$8de92c3_3_18;
	r6 = 0;
	if( r6 >= r3 ) goto label$8de92c3_3_18;
	r8 = (haxe__io__Eof)hl_alloc_obj(&t$haxe_io_Eof);
	haxe_io_Eof_new(r8);
	hl_throw((vdynamic*)r8);
	label$8de92c3_3_18:
	r5 = r0->len;
	if( r5 >= r3 ) goto label$8de92c3_3_22;
	r5 = r0->len;
	r3 = r5;
	label$8de92c3_3_22:
	r9 = r1->b;
	r10 = r0->b;
	r6 = r0->pos;
	hl_bytes_blit(r9,r2,r10,r6,r3);
	r5 = r0->pos;
	r5 = r5 + r3;
	r0->pos = r5;
	r5 = r0->len;
	r5 = r5 - r3;
	r0->len = r5;
	return r3;
}

